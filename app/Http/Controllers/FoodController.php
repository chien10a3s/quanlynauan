...
class FoodController extends Controller
{
    ...
    {
        $val_search = Input::get('val_search');
        $idclick = Input::get('name_click');
        $cl = Input::get('cl');

//        $all_food = Food::with('category')->where('name', 'like', '%' . $val_search . '%')->where('status', 1)->get();
        $option = [];
        $all_food = Food::with('supplier')->where('name', 'like', '%' . $val_search . '%')->where('status', 1)->get();
        if (count($all_food) > 0) {
            foreach ($all_food as $item_food) {
                ...
                $option[$name][$item_food->id]['name'] = $item_food->name;
                $option[$name][$item_food->id]['price'] = number_format($item_food->price) . ' VND / '.$item_food->unit;
                $option[$name][$item_food->id]['image'] = $item_food->image;
            }
        }
        return view('meal.ajax_search',compact('option','idclick','cl'));
    }
}
