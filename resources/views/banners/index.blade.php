...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i> Danh sách
    </h1>
    &nbsp;
                        ...
                        <table id="sample_1" class="table table-hover">
                            ...
                            @if(count($banners) > 0)
                                ...
                                @foreach($banners as $banner)
                                    <tr>
                                        <td>{{ $i+=1 }}</td>
                                        <td><img width="100" src="{{ Voyager::image( $banner->image ) }}" /></td>
                                        <td>{{ $banner->url }}</td>
                                        <td>
                                            @if($banner->active == 1)
                                                Hoạt động
                                        ...
                                        <td>
                                            <a href="{{ route('banner.edit',$banner->id) }}" title="Chỉnh sửa"
                                               class="btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> Sửa
                                            </a>

                                            <div class="btn-sm btn-danger pull-right delete" data-id="{{ $banner->id }}" id="delete-{{ $banner->id }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('banner.index') }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
