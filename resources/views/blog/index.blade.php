...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i> Danh sách Blog
    </h1>
    &nbsp;
                        ...
                        <table id="sample_1" class="table table-hover">
                            ...
                            @if(count($blogs) > 0)
                                ...
                                @foreach($blogs as $blog)
                                    <tr>
                                        <td>{{ $i+=1 }}</td>
                                        <td><img src="{{ Voyager::image( $blog->featured_image ) }}" /></td>
                                        <td>{{ $blog->title }}</td>
                                        <td>{{ $blog->slug }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('blog.edit',$blog->id) }}" title="Chỉnh sửa"
                                               class="btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> Sửa
                                            </a>
                                            
                                            <div class="btn-sm btn-danger pull-right delete" data-id="{{ $blog->id }}" id="delete-{{ $blog->id }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i> Bạn chắc chắn muốn xóa blog này?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('blog.index') }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
