...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i> Danh sách đánh giá của khách hàng
    </h1>
    &nbsp;
                        ...
                        <table id="sample_1" class="table table-hover">
                            ...
                            @if(count($testimonials) > 0)
                                ...
                                @foreach($testimonials as $testimonial)
                                    <tr>
                                        <td>{{ $i+=1 }}</td>
                                        <td><img width="100" src="{{ Voyager::image( $testimonial->image ) }}" /></td>
                                        ...
                                        <td>
                                            <a href="{{ route('testimonial.edit',$testimonial->id) }}" title="Chỉnh sửa"
                                               class="btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> Sửa
                                            </a>

                                            <div class="btn-sm btn-danger pull-right delete" data-id="{{ $testimonial->id }}" id="delete-{{ $testimonial->id }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('testimonial.index') }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
